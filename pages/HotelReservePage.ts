...
export default class HotelReservePage {
  ...
  constructor(page: Page) {
    ...
    this.successModalTitle = page.locator('h5.modal-title', { hasText: 'Thank you for reserving.' });
  ...
  async fillForm(planData: any, guestData: any) {
    ...
    if (planData.wantsBreakfast) { await this.breakfastCheck.check(); }
    if (planData.wantsEarlyCheckIn) { await this.earlyCheckInCheck.check(); }
    if (planData.wantsSightseeing) { await this.sightseeingCheck.check(); }
